...
public class Egg : MonoBehaviour {
	...
	void BossStages(){
		...
		if(healthRatio == 1.0f){
			if (!stage1)
			{
				...
				InvokeRepeating("SpawnEggs", 0.01f, 3f);
				stage1 = true;
			}
			...
			if (stage2)
			{
				if (enemyNum == 0)
				{
					...
					{       
						...
						isHiding = false;
						movementAnim.SetBool("isHiding", isHiding);
						InvokeRepeating("SpawnEggs", 0.01f, 2f);
					}
				}
			}
        }
            ...
            if (stage5)
            {
                if (enemyNum == 0)
                {
                    if (isHiding)
                    {
                        isHiding = false;
                        movementAnim.SetBool("isHiding", isHiding);
						InvokeRepeating("SpawnEggs", 0.01f, 0.5f);
                    }
                }
            }
            ...
            if (stage6)
            {
                if (enemyNum == 0)
                {
                    if (isHiding)
                    {
                        isHiding = false;
                        movementAnim.SetBool("isHiding", isHiding);
						InvokeRepeating("SpawnEggs", 0.01f, 0.25f);
                    }
                }
            }
        }
	...
	public void SpawnEggs(){
		...
		Vector2 eggDirection = new Vector2(Random.Range(-10,10), Random.Range(2,5));
